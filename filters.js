...
export function updateFilterOptions() {
    ...
    updateSelectOptions('bookFilter', books);

    const sports = [...new Set(this.data.map(d => d.sport).filter(Boolean))].sort();
    updateSelectOptions('sportFilter', sports);
}
...
export function setupColumnFilters() {
...
export function getCellValue(record, colIndex) {
    const values = [
...
export function getSortValue(record, column) {
    switch (column) {
